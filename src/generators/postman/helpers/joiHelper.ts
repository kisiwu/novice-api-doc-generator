...
export class PostmanJoiHelper extends JoiHelper implements PostmanHelperInterface {
  getFirstItem(): PostmanJoiHelper | undefined {
    ...
    if (this._joi.$_terms
      ...
      r = new PostmanJoiHelper({ value: this._joi.$_terms.items[0] });
    }
    return r;
  }
  ...
  getChildren(): Record<string, PostmanJoiHelper> {
    ...
    if (this._joi.$_terms
      ...
      this._joi.$_terms.keys.forEach(
        (c: { key: string, schema?: Record<string, JoiSchema> }) => r[c.key] = new PostmanJoiHelper({ value: c.schema }));
    }
    return r;
  }
  ...
  getAlternatives(): PostmanJoiHelper[] {
    ...
    if (this._joi.$_terms
      ...
      this._joi.$_terms.matches.forEach(
        (c: { schema?: JoiSchema; }) => {
          if (c.schema) {
            r.push(new PostmanJoiHelper({ value: c.schema }))
          }
        }
      );
    }
